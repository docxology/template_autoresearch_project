...
`{{PROJECT_NAME}}` shows how a tractable AutoResearch task can be represented as
reproducible template infrastructure. The `{{ML_TASK_NAME}}` experiment is small
...
validation, local integrity attestation (`{{SECURITY_ATTESTATION_STATUS}}`),
